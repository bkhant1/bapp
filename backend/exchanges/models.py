...
class BookExchange(models.Model):
    ...
    requested_book = models.ForeignKey(
        "books.UserBook", on_delete=models.CASCADE, related_name="exchange_requests"
    )
    ...
    exchange_type = models.CharField(
        max_length=20, choices=EXCHANGE_TYPE_CHOICES, default="permanent"
    )
...
class ExchangeRating(models.Model):
    ...
    exchange = models.ForeignKey(
        BookExchange, on_delete=models.CASCADE, related_name="ratings"
    )
    ...
    book_condition_rating = models.PositiveIntegerField(
        choices=[(i, i) for i in range(1, 6)], blank=True, null=True
    )
...
class ExchangeMessage(models.Model):
    ...
    exchange = models.ForeignKey(
        BookExchange, on_delete=models.CASCADE, related_name="messages"
    )
